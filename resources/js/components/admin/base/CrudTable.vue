...
<template>
	<b-card ref="bCard">
		<template slot="header">
			<div class="row">
				<div :id="`${resourceName.toLowerCase()}-title`" class="d-flex col-4 align-items-center">
		...
		<b-table :id="`${resourceName.toLowerCase()}-table`" ref="bTable" :items="items" :fields="fields" :filter="searchText" :current-page="currentPage"
	...
	export default {
		...
		methods: {
			...
			isColumnSearch(searchText) {
				...
				matches = (matches) ? matches.map(match => match.replace(/[\[\]]/g, "").split(":")).flat() : [];
